...
public class CT1_BoxController : MonoBehaviour
{
    ...
    private CT1_PiecePosition piecePosition;
    ...
    void Start()
    {
        ...
        piecePosition = gameCanvas.GetComponent<CT1_PiecePosition>();
    }
    ...
    bool ifOverLap()
    {
        if(xPos == player.GetComponent<CT1_PlayerController>().xPos && yPos == player.GetComponent<CT1_PlayerController>().yPos)
        {
            return true;
        }
        return false;
    ...
    {
        lastPlayerMovement = player.GetComponent<CT1_PlayerController>().attemptMovement;
        getPushed = true;
        if (thereIsObstacle())
        {
            player.GetComponent<CT1_PlayerController>().rebound();
        }
        else
        {
            move();
            //    piecePosition.addBoxPos(player.GetComponent<CT1_PlayerController>().attemptMovement);
            positionHistory[positionHistory.Count - 1] = new List<int> { xPos, yPos };
            movementHistory[movementHistory.Count - 1] = lastPlayerMovement;
        }
        ...
        {
            gameObject.GetComponentInParent<CT1_BoxManager>().checkIfWin();
            answerQuestion();
        }
    ...
    IEnumerator checkIfBug()
    {
        ...
        bool condition2 = xPos == player.GetComponent<CT1_PlayerController>().xPos && yPos == player.GetComponent<CT1_PlayerController>().yPos;
        if (condition2)
        {
            //Debug.Log("Box Position: ["+xPos+","+yPos+"]");
            //Debug.Log("Player Position: [" + player.GetComponent<CT1_PlayerController>().xPos + "," + player.GetComponent<CT1_PlayerController>().yPos + "]");
            player.GetComponent<CT1_PlayerController>().rebound();
        }
    }
    ...
    public void answerQuestion()
    {
        ...
        gameObject.GetComponent<CT1_QuestionBoxCondition>().checkBoxQuestionStatus();
    ...
    //    printArray(movementHistory, "Box Movement History: ");
        ...
        {
            Debug.Log("Error in reverseBoxMove in CT1_BoxController");
        }
    }
    ...
    IEnumerator buffer()
    {
        ...
        GameObject.Find("Player").GetComponent<CT1_PlayerController>().enabled = false;
    }
}
